...
void main(void)                                   // Main program entry point
{
	...
	// DEVICE_DT_GET_ONE ensures there is exactly one compatible device.
	const struct device *sensor = DEVICE_DT_GET_ONE(st_lis2dh);   // Get device handle for LIS2DH accelerometer

	if (sensor == NULL) {                        // Check if sensor device was found
		printf("Could not get %s device\n", "st_lis2dh");   // Print error if device not found
